...
def load_state(repo_root="."):
    ...
    state = {
        ...
        "thresholds": {"auto_fix_enable": 100},
    }
    try:
...
def write_insights(
    report, state, out_path=".logs/validation/md_autofix_agent_insights.json"
):
    os.makedirs(os.path.dirname(out_path), exist_ok=True)
    insights = {
        "summary": report.get("summary", {}),
        "top_problems": sorted(
            report.get("problem_counts", {}).items(), key=lambda x: -x[1]
        )[:40],
...
def renumber_ordered_blocks(out, path, fixed):
    """
    Renumber ordered-list blocks in-place within `out` (list of lines).
    Allows a single blank line between items and skips fenced code blocks.
...
def process_file(path, state=None):
        ...
        if m_any:
                    ...
                    fixed.append(
                        {
                            "file": path,
                            "line": i + 1,
                            "fix": "normalize closing fenced block marker",
                        }
                    )
            ...
            if (not in_fence) and fence_open_re.match(line):
                ...
                if action == "auto-fix":
                    ...
                    problems.append(
                        {"file": path, "line": i + 1, "problem": problem_name}
                    )
                    fixed.append(
                        {
                            "file": path,
                            "line": i + 1,
                            "fix": "add language 'text' to fenced block",
                        }
                    )
                else:
                    problems.append(
        ...
        if m:
                        ...
                        problems.append(
                            {
                                "file": path,
                                "line": i + 1,
                                "problem": "extra top-level heading converted to H2",
                            }
                        )
                        fixed.append(
                            {
                                "file": path,
                                "line": i + 1,
                                "fix": "convert H1 to H2 to satisfy single-title rule",
                            }
                        )
                    else:
                        problems.append(
            ...
            if i + 1 < len(out) and out[i + 1].strip() != "":
                ...
                if action == "auto-fix":
                    out.insert(i + 1, "")
                    problems.append(
                        {"file": path, "line": i + 2, "problem": problem_name}
                    )
                    fixed.append(
                        {
                            "file": path,
                            "line": i + 2,
                            "fix": "insert blank line after heading",
                        }
                    )
                    i += 1
                else:
                ...
                problems.append(
                    {
                        "file": path,
                        "line": j + 1,
                        "problem": "missing blank line after list",
                    }
                )
                fixed.append(
    ...
    # Consolidated renumber pass: handle all ordered-list blocks robustly
    out = renumber_ordered_blocks(out, path, fixed)
    new_text = "\n".join(out) + "\n"
    if not had_trailing:
        problems.append(
            {"file": path, "line": len(out), "problem": "missing trailing newline"}
        )
    ...
    if path.lower().endswith(".md"):
        if m_local not in new_text:
            new_text = new_text.rstrip("\n") + "\n\n" + m_local + "\n"
            fixed.append(
                {
                    "file": path,
                    "line": len(out) + 2,
                    "fix": "append md_autofix trigger marker",
                }
            )
